...
export function DifficultyBadge({ difficulty, size = 'normal', className = '' }: DifficultyBadgeProps) {
  ...
  const sizeClasses = size === 'small' ? 'gap-1 px-2 py-0.5 text-xs' : 'gap-1.5 px-3 py-1.5 text-sm';

  return (
    <span
      className={`inline-flex items-center rounded-full font-semibold shadow-md transition-all duration-200 ${sizeClasses} ${config.color} ${config.hoverColor} ${config.textColor} ${className}`}
